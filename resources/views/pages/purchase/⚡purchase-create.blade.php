...
new class extends Component
{
    ...
    public function save()
    {
        ...
        $budget = $this->findBudgetPlan();

        if (!$budget) {
            $this->addError('date', 'Tidak ada Budget Plan untuk tanggal ini.');
            return;
        }
        ...
        foreach ($this->items as $item) {
            if (!$item['vendor_id']) {
                $this->addError("items.$index.vendor_id", "Vendor wajib dipilih");
                return;
            }
...
<div class="space-y-6">
    ...
    <x-header title="Buat Purchase Order" subtitle="Belanja bahan harian" separator />

    <x-card title="Informasi PO" separator>
        <div class="grid grid-cols-3 gap-4">
            <x-input type="date" label="Tanggal PO" wire:model="date" />


            <x-select
                label="Referensi Menu (opsional)"
                :options="$this->menus->map(fn($m)=>['id'=>$m->id,'name'=>$m->name.' - '.$m->date])"
                wire:model.live="menu_id" placeholder="...">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td>
                        <x-input
                            ...
                            value="{{ number_format($item['qty_display'] * $item['price'], 0, ',', '.') }}"
                ...
                @endforeach
                    ...
                    <tr class="border-t border-slate-200">
                        <td colspan="4" class="text-right font-semibold pr-4">
                        </td>
                        <td>
    ...
    <div class="flex justify-end">
        <x-button label="Simpan PO" wire:click="save" class="btn-primary" />
    </div>
