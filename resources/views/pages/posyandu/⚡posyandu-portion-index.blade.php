<?php

use Livewire\Component;
use App\Models\Posyandu;
use App\Models\PosyanduPortion;

new class extends Component
{
    public $breadcrumbs = [];

    public function mount()
    {
        $this->breadcrumbs = [
            ['icon' => 'o-home', 'link' => route('dashboard')],
            ['label' => 'Posyandu', 'link' => route('posyandu.index')],
            ['label' => 'Porsi Posyandu'],
        ];
    }

    public function getPosyanduPortionsProperty()
    {
        return PosyanduPortion::with([
            'posyandu' => function ($q) {
                $q->withTrashed();
            }
        ])->latest()->get();

    }
};
?>

<div>
    <x-breadcrumbs :items="$breadcrumbs" class="mb-3" />

    <x-header title="Histori Porsi" subtitle="Log Perubahan Porsi" separator>
        <x-slot:actions>
            <x-button link="{{ route('posyandu.index') }}" route="posyandu.index" label="Daftar Posyandu" />
        </x-slot:actions>
    </x-header>

    {{-- Filter & Search Section --}}
    <div class="bg-white p-4 rounded-2xl shadow-sm border border-slate-100 mb-6">

        <div class="grid md:grid-cols-4 gap-4 items-start">

            {{-- Search --}}
            <span class="col-span-1 md:col-span-2">
                <x-input label="Cari Posyandu" icon="o-magnifying-glass" placeholder="Ketik nama sekolah..." wire:model="search" clearable>
                    <x-slot:append>
                        {{-- Add `join-item` to all appended elements --}}
                        <x-button icon="o-magnifying-glass" class="join-item btn-primary" wire:click="getPosyanduPortionsProperty" />
                    </x-slot:append>
                </x-input>
                
            </span>

        </div>

    </div>

    {{-- Log Porsi --}}
    <ul class="space-y-3">
        @foreach ($this->posyandu_portions as $item)
            <li class="bg-white p-4 rounded-lg shadow-sm">

                {{-- Header --}}
                <div class="mb-3">
                    <h3>
                        @if ($item->posyandu->trashed())
                        {{ $item->posyandu->posyandu_name }} - {{ $item->posyandu->posyandu_code }} <span class="font-light text-sm text-red-500">(Posyandu Tidak Aktif)</span>
                        @else
                        {{ $item->posyandu->posyandu_name }} - {{ $item->posyandu->posyandu_code }}
                        @endif
                    </h3>
                    <h5 class="text-xs text-slate-500">
                        {{ $item->created_at->format('D, d M Y H:i') }}
                    </h5>
                </div>

                {{-- Portion Grid --}}
                <div class="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">

                    @foreach ([
                        'Bumil' => $item->bumil,
                        'Busui' => $item->busui,
                        'Balita' => $item->balita,
                    ] as $label => $port)

                    {{-- POSITIF --}}
                    @if ($port > 0)
                        <x-stat
                        title="{{ $label }} - Naik"
                        value="{{ $port > 0 ? '+' . $port : $port }}"
                        icon="o-arrow-trending-up"
                        color="text-green-500" />

                    {{-- NOL --}}
                    @elseif ($port == 0)
                        <x-stat
                        title="{{ $label }} - Tetap"
                        value="{{ $port > 0 ? '+' . $port : $port }}"
                        icon="o-minus"
                        color="text-gray-500" />

                    {{-- NEGATIF --}}
                    @else
                        <x-stat
                        title="{{ $label }} - Turun"
                        value="{{ $port > 0 ? '+' . $port : $port }}"
                        icon="o-arrow-trending-down"
                        color="text-red-500" />
                    @endif

                    @endforeach

                </div>
            </li>
        @endforeach
    </ul>
</div>
